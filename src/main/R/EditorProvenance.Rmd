...
```{r}
small_fit <- glm(other_total/project_total ~ wplangshare + logjournals2, family = binomial(logit), data=top_df);
summary(small_fit);
```
...
We now try to model the remaining deviance (top-level explains 69% of variance):
...
```{r}
small_fit <- glm(count/article_total ~ top_level +  countrylangshare +   logmigration1 + is_local, family = binomial(logit), data=df);
summary(small_fit);
```
