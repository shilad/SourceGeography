 ...
 * The country associated with the publisher of a source (pc).

We count the number of citations that occur for each possible triple, and find all possible attributes associated with pc, ac, and wplang significant.
...
For example: Across all articles located in Japan (ac) in the Chinese Wikipedia (wplang), how many are published by Japan (pc).
...
```{r}
small_fit <- glm(cbind(count, article_total - count) ~ logjournals1 + logPOP + article_is_native, family = binomial(logit), data=local_df)
summary(small_fit);
```
...
```{r}
small_fit <- glm(cbind(other_total, project_total-other_total) ~ logjournals2 + wplangshare, family = binomial(logit), data=top_df);
summary(small_fit);
```
...
```{r}
full <- glm(count / article_total ~ . - article_country - other_country - project -project_total -article_total - country, family = binomial(logit), data=df);
summary(full);
```
...
```{r}
min_fit <- glm(count/ article_total ~ 1, family = binomial(logit), data=df);
...
```{r}
s <- step(min_fit, scope=formula(full), direction='forward', trace=FALSE);
s$anova;
```

```{r}
small_fit <- glm(cbind(count, article_total-count) ~ top_level + logdist + logjournals2, family = binomial(logit), data=df);
summary(small_fit);
```
